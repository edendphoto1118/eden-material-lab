document.addEventListener('DOMContentLoaded', function() {
    ...
    if(downloadBtn) {
        downloadBtn.addEventListener('click', function() {
            // 注意：這裡改抓 .magazine-cover 而不是 wrapper，避免截到外圍空白
            const targetElement = document.querySelector('.magazine-cover'); 
            const btn = this;
            ...
            html2canvas(targetElement, {
                ...
                allowTaint: true,
                backgroundColor: "#fff",
                // [核心技術] onclone: 在截圖產生前，對複製的 DOM 進行「整形」
                onclone: (clonedDoc) => {
                    const clonedCard = clonedDoc.querySelector('.magazine-cover');
                    ...
                    // 1. 處理雜點 (解決霧化)
                    // 如果直接 none，質感會消失；如果不 none，會變白霧。
                    // 最佳解：隱藏 filter 雜點，因為截圖引擎無法完美渲染 SVG filter
                    if(noise) {
                        noise.style.display = 'none'; 
                    }

                    // 2. 處理陰影 (解決黑塊)
                    // 將 multiply 混合模式改為普通模式，並調整透明度模擬
                    if(texture) {
                        texture.style.mixBlendMode = 'normal';
                        // 用 CSS 漸層模擬原本的 multiply 效果，這樣截圖引擎就能讀懂
                        texture.style.background = 'linear-gradient(to bottom, transparent 40%, rgba(0,0,0,0.5) 100%)'; 
                    }

                    // 3. 處理照片濾鏡 (解決雙重濾鏡導致的過曝/模糊)
                    if(imgLayer) {
                        imgLayer.style.filter = 'none'; // 移除 CSS 濾鏡，直接呈現原圖
                    }

                    // 4. 強制移除 3D 變形 (解決位移與模糊)
                    if(clonedCard) {
                ...
                link.download = 'EDEN_FACADE_REPORT_2026.jpg'; // 使用 JPG 格式
                link.href = canvas.toDataURL("image/jpeg", 0.95); // 高品質 JPG
                link.click();
